...
export const E2E_TEST_DIR = path.join(Deno.cwd(), "assurance");
export const ZIP_URL =
    "https://synthetichealth.github.io/synthea-sample-data/downloads/10k_synthea_covid19_csv.zip";
export const ZIP_FILE = path.join(E2E_TEST_DIR, "10k_synthea_covid19_csv.zip");
export const INGEST_DIR = path.join(E2E_TEST_DIR, "ingest");
...
Deno.test("file ingestion", async (t) => {
    ...
    await t.step("file change history", async () => {
        ...
        assertAlmostEquals(files, filesInDir, 10);
    });
